<?php
class Register_15 extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('Loginregister_15');
    }

    public function index()
    {
        $this->form_validation->set_rules('email', 'email', 'required|valid_email');
        $data['perans'] = $this->Loginregister_15->get_perans();
        $this->load->view('login/view_register_15', $data);
    }
        ...
        if (($email != '') and ($namaPengguna != '') and ($password != '') and ($konfirmpassword != '')) {
            ...
            if ($tes_duplikat > 0) {
                ...
                if ($password == $konfirmpassword) {
                    ...
                    $this->Loginregister_15
                        ->tambah_user($data);

                    //ambil data id user dan id peran
                    $dataUser = $this->Loginregister_15
                    ...
                    $this->load->view('login/view_register_15');
                }
            }
        } else {
            redirect('register_15');
