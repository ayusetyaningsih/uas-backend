<?php
class Manage_contributor_15 extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('Contributor_15');
    }
    ...
    public function proses_tambah_kategori()
    {
        ...
        if ($name_kategori != '') {
            ...
            $this->Contributor_15->tambah_kategori($data);

            redirect('zone_contributor_15');
        } else {
            redirect('manage_contributor_15/tambah_kategori');
        }
    }
    ...
    public function edit_kategori($id)
    {
        ...
        $row = $this->Contributor_15->ambil_id_kategori($id_encrypt);
    ...
    public function proses_edit_kategori()
    {
        ...
        if ($nama_kategori != '') {
            ...
            $this->Contributor_15->edit_kategori($data);
            redirect('zone_contributor_15');
        } else {
            redirect('zone_contributor_15');
        }
    ...
    {
        $this->Contributor_15->ambil_id_kategori($id);
        $this->Contributor_15->delete_kategori($id);
        redirect('zone_contributor_15');
    }
    ...
    public function berita()
    {
        ...
        // $dataKategori = $this->Contributor_15->ambil_data_kategori();

        $total_row = $this->Contributor_15->total_row();
        $config['base_url'] = base_url() . 'Manage_contributor_15/berita/';
        $config['total_rows'] = $total_row;
        $config['per_page'] = 4;
        $awal = $this->uri->segment(4);

        $this->pagination->initialize($config);
        $data_page = $this->Contributor_15->berita_page($config['per_page'], $awal);
    ...
    public function tambah_berita()
    {
        ...
        $querymenu = "SELECT * FROM `trx_menu` INNER JOIN `menu` ON `trx_menu`.`id_menu` = `menu`.`id_menu` WHERE `trx_menu`.`id_peran` = $id_peran";
        $menu['menus'] = $this->db->query($querymenu)->result_array();
        $data['kategoris'] = $this->Contributor_15->get_kategori();
    ...
    public function proses_tambah_berita()
    {
        ...
        if (($judul_berita != '') and ($isi_berita != '')) {
            ...
            $this->Contributor_15->tambah_berita($data);
            ...
            $this->Contributor_15->tambah_berita_kategori($data2);
            // var_dump($id);
            // die;

            redirect('manage_contributor_15/berita');
        } else {
            redirect('manage_contributor_15/tambah_berita');
        }
    }
    ...
    public function edit_berita($id)
    {
        ...
        $row = $this->Contributor_15->ambil_id_berita($id);
        // var_dump($row);
        // die;
    ...
    public function proses_edit_berita()
    {
        ...
        if ($judul_berita != '' && $isi_berita != '') {
            ...
            // var_dump($data);
            // die;
            $this->Contributor_15->edit_berita($data);
            redirect('manage_contributor_15/berita');
        } else {
            $id_peran = 2;
            $querymenu = "SELECT * FROM `trx_menu` INNER JOIN `menu` ON `trx_menu`.`id_menu` = `menu`.`id_menu` WHERE `trx_menu`.`id_peran` = $id_peran";
    ...
    {
        $this->Contributor_15->ambil_id_berita($id);
        $this->Contributor_15->delete_berita($id);
        redirect('manage_contributor_15/berita');
    }
}
